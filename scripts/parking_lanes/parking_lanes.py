...
vehicle_length = 4.4        # average vehicle length (a single vehicle, without maneuvering distance)
...
def prepareLayers():
    ...
    for side in ['left', 'right']:
        ...
        for id in range(attributes - 1, 0, -1):
            if layer.attributeDisplayName(id) not in parking_key_list and 'parking:lane' not in layer.attributeDisplayName(id) and 'parking:condition' not in layer.attributeDisplayName(id):
        ...
        for attr in parking_key_list:
            if attr != 'highway' and attr != 'error_output':
                layer.renameAttribute(layer.fields().indexOf(attr), f'highway:{attr}')
...
def fillBaseAttributes(layer, commit):
    ...
    for feature in layer.getFeatures():
            ...
            if parking_orientation:
                ...
                if dir_both:
                    if position and feature.attribute('parking:lane:both:' + parking_orientation):
                        error += f'[pl02{side[:1]}] Attribute "parking:lane:{side}:{parking_orientation}" und "parking:lane:both:{parking_orientation}" gleichzeitig vorhanden. '
                    if not position:
        ...
        parking_left_width = parking_right_width = 0
...
def prepareParkingLane(layer, side, clean):
    ...
    Args:
        ...
        clean: (True/False) Gibt an, ob die Attributtabelle nach Abschluss der Berechnungen von überflüssigen Einzelattributen des Rohdatensatzes bereinigt werden soll. Insbesondere für die Fehlersuche im Rohdatensatz kann es hilfreich sein, die Attribute zum Vergleich zu behalten.
    """
    ...
    cond_side = f'parking:condition:{side}' in layer.attributeAliases()
    cond_both = 'parking:condition:both' in layer.attributeAliases()
    cond_default_side = f'parking:condition:{side}:default' in layer.attributeAliases()
    cond_default_both = 'parking:condition:both:default' in layer.attributeAliases()
    cond_time_side = f'parking:condition:{side}:time_interval' in layer.attributeAliases()
    cond_time_both = 'parking:condition:both:time_interval' in layer.attributeAliases()
    vehicles_side = f'parking:condition:{side}:vehicles' in layer.attributeAliases()
    vehicles_both = 'parking:condition:both:vehicles' in layer.attributeAliases()
    maxstay_side = f'parking:condition:{side}:maxstay' in layer.attributeAliases()
    maxstay_both = 'parking:condition:both:maxstay' in layer.attributeAliases()
    capacity_side = f'parking:lane:{side}:capacity' in layer.attributeAliases()
    ...
    for feature in layer.getFeatures():
        ...
        readable_attributes = [
            'parking:lane:both',
            f'parking:lane:{side}',
            f'parking:lane:{side}:capacity',
            'parking:lane:both:capacity',
            f'parking:lane:{side}:width',
            'parking:lane:both:width',
            f'parking:lane:{side}:width:carriageway',
            f'parking:lane:{side}:position',
            f'parking:lane:{side}:offset',
            f'parking:condition:{side}',
            'parking:condition:both',
            f'parking:condition:{side}:default',
            'parking:condition:both:default',
            f'parking:condition:{side}:time_interval',
            'parking:condition:both:time_interval',
            f'parking:condition:{side}:vehicles',
            'parking:condition:both:vehicles',
            f'parking:condition:{side}:maxstay',
        ...
        for attr in layer_parking_left.attributeAliases():
            if 'parking:lane' in attr or 'parking:condition' in attr:
                if side == 'left':
                    if ('right' not in attr) and (attr not in readable_attributes):
                        if feature.attribute(attr):
                            error += '[ig_al] Attribut "' + attr + '" nicht berücksichtigt. '
                elif side == 'right':
                    if ('left' not in attr) and (attr not in readable_attributes):
                        if feature.attribute(attr):
...
# TODO: enum benutzen anstelle eines string-switches
def getIntersections(layer, method: str):
    ...
    Args:
        ...
        method (str): ('full', 'fast') Die Berechnungsmethode, mit der vorgegangen wird.
        * `full`: Ermittelt alle Schnittpunkte des Straßenlayers mit sich selbst, an denen mehr als zwei Segmente zusammentreffen. Vollständige, aber rechenaufwendige Methode.
        * `fast`: Wesentlich schnellere Variante, bei der mit einem nach Straßennamen aufgelösten Straßenlayer gerechnet wird. In Einzelfällen können hierbei jedoch Kreuzungen fehlen, wenn die sich kreuzenden Straßen den selben Namen tragen.
    """

    if method != 'fast':
        # Straßen in Einzelteile zerlegen, um die Anzahl abgehender Wege sicher für jede Kreuzung ermitteln zu können
        expl_street = processing.run(
            'native:explodelines',
            {
                'INPUT': layer,
                'OUTPUT': 'memory:'
        ...
        intersections = processing.run(
            ...
            {
                'INPUT': expl_street,
                'INTERSECT': expl_street,
                'OUTPUT': dir + 'data/buffer_points/intersections.geojson'
            }
        )['OUTPUT']
        ...
        for feature in intersections.getFeatures():
            ...
            processing.run(
                'native:selectbylocation',
                {
                    ...
                    'INTERSECT': QgsProcessingFeatureSourceDefinition(
                        dir + 'data/buffer_points/intersections.geojson',
            ...
            width = 0
            del_features = []  # noqa  # F841  # variable unused
...
def getKerbIntersections(layer):
    ...
    # layer_diss_streets = processing.run(
    #     'native:dissolve',
    #     {
    #         'FIELD': [
    #             'name',
    ...
    #     }
    # )['OUTPUT']
    layer_streets_intersect = processing.run(
        'native:lineintersections',
        {
            'INPUT': layer_diss_streets,
            'INTERSECT': layer_diss_streets,
            'OUTPUT': 'memory:'
        }
    )['OUTPUT']
    layer_streets_intersect_buffer = processing.run(
        'native:buffer',
        {
            'DISTANCE': 0.1,
            'INPUT': layer_streets_intersect,
            'OUTPUT': 'memory:'
        }
    )['OUTPUT']
    layer_streets_diff = processing.run(
        'native:difference',
        {
            'INPUT': layer_diss_streets,
            'OVERLAY': layer_streets_intersect_buffer,
            'OUTPUT': 'memory:'
        }
    )['OUTPUT']

    # Bordsteinlinien durch Versatz simulieren
    layer_kerb_left = processing.run(
        'native:offsetline',
        {
            'INPUT': layer_streets_diff,
            'DISTANCE': QgsProperty.fromExpression('"parking:lane:left:offset"'),
            'OUTPUT': 'memory:'
        }
    )['OUTPUT']
    layer_kerb_right = processing.run(
        'native:offsetline',
        {
            'INPUT': layer_streets_diff,
            'DISTANCE': QgsProperty.fromExpression('"parking:lane:right:offset"'),
            'OUTPUT': 'memory:'
        }
    )['OUTPUT']
    ...
    layer_streets_intersect1 = processing.run(
        'native:lineintersections',
        {
            'INPUT': layer_kerb_left,
            'INTERSECT': layer_kerb_right,
            'OUTPUT': 'memory:'
        }
    )['OUTPUT']
    layer_streets_intersect2 = processing.run(
        'native:lineintersections',
        {
            'INPUT': layer_kerb_left,
            'INTERSECT': layer_kerb_left,
            'OUTPUT': 'memory:'
        }
    )['OUTPUT']
    layer_streets_intersect3 = processing.run(
        'native:lineintersections',
        {
            'INPUT': layer_kerb_right,
            'INTERSECT': layer_kerb_right,
            'OUTPUT': 'memory:'
        }
    )['OUTPUT']
    ...
    layer_streets_intersect = processing.run(
        ...
        {
            'LAYERS': [
                layer_streets_intersect1,
                layer_streets_intersect2,
                layer_streets_intersect3
            ],
            'OUTPUT': 'memory:'
        }
    )['OUTPUT']
...
def bufferIntersection(layer_parking, layer_intersect, buffer, buffer_name, intersects):
    ...
    Args:
        ...
        buffer: Ausdruck/Formel zur Ermittlung des Radius, der freigehalten werden soll
        buffer_name: Wenn angegeben, wird der erzeugte Puffer mit diesem Namen der Karte hinzugefügt
        intersects (optional): Wenn hier ein Punktlayer ^übergeben wird, wird dieser gepuffert, statt die Kreuzungspunkte aus den ersten beiden Layern zu berechen.
    """

    if not intersects:
        intersects = processing.run(
            'native:lineintersections',
            {
                'INPUT': layer_parking,
                'INTERSECT': layer_intersect,
                'OUTPUT': 'memory:'
            }
        )['OUTPUT']

    intersects_buffer = processing.run(
        'native:buffer',
        {
            'DISTANCE': QgsProperty.fromExpression(buffer),
            'INPUT': intersects,
            'OUTPUT': 'memory:'
        }
    )['OUTPUT']
    layer_parking = processing.run(
        'native:difference',
        {
            'INPUT': layer_parking,
            'OVERLAY': intersects_buffer,
            'OUTPUT': 'memory:'
        }
    )['OUTPUT']
...
def bufferCrossing(layer_parking, layer_crossing, side):
    ...
    Args:
        ...
        side (_type_): _description_  # TODO
    """
    ...
    processing.run(
        ...
        {
            'INPUT': layer_crossing,
            'EXPRESSION': """"highway" = 'traffic_signals' AND "traffic_signals:direction" IS NOT 'forward' AND "traffic_signals:direction" IS NOT 'backward'"""
        }
    )
    buffer01 = processing.run(
        'native:buffer',
        {
            'DISTANCE': 10,
    ...
    if side == 'left':
        processing.run(
            ...
            {
                'INPUT': layer_crossing,
                'EXPRESSION': """"highway" = 'traffic_signals' AND "traffic_signals:direction" = 'backward'"""
            }
        )
    if side == 'right':
        processing.run(
            ...
            {
                'INPUT': layer_crossing,
                'EXPRESSION': """"highway" = 'traffic_signals' AND "traffic_signals:direction" = 'forward'"""
            }
        )
    buffer_s01 = processing.run(
        'native:buffer',
        {
            'DISTANCE': 10,
            'INPUT': QgsProcessingFeatureSourceDefinition(
    ...
    processing.run(
        ...
        {
            'INPUT': layer_crossing,
            'EXPRESSION': """"crossing:kerb_extension" = 'both' OR "crossing:buffer_marking" = 'both'"""
        }
    )
    buffer02 = processing.run(
        'native:buffer',
        {
            'DISTANCE': 3,
            'INPUT': QgsProcessingFeatureSourceDefinition(
    ...
    if side == 'left':
        processing.run(
            ...
            {
                'INPUT': layer_crossing,
                'EXPRESSION': """"crossing:kerb_extension" = 'left' OR "crossing:buffer_marking" = 'left'"""
            }
        )
    if side == 'right':
        processing.run(
            ...
            {
                'INPUT': layer_crossing,
                'EXPRESSION': """"crossing:kerb_extension" = 'right' OR "crossing:buffer_marking" = 'right'"""
            }
        )
    buffer_s02 = processing.run(
        'native:buffer',
        {
            'DISTANCE': 3,
            'INPUT': QgsProcessingFeatureSourceDefinition(
    ...
    processing.run(
        ...
        {
            'INPUT': layer_crossing,
            'EXPRESSION': """"crossing" = 'zebra' OR "crossing_ref" = 'zebra' OR "crossing" = 'traffic_signals'"""
        }
    )
    buffer03 = processing.run(
        'native:buffer',
        {
            'DISTANCE': 4.5,
            'INPUT': QgsProcessingFeatureSourceDefinition(
    ...
    processing.run(
        ...
        {
            'INPUT': layer_crossing,
            'EXPRESSION': """"crossing" = 'marked'"""
        }
    )
    buffer04 = processing.run(
        'native:buffer',
        {
            'DISTANCE': 2,
            'INPUT': QgsProcessingFeatureSourceDefinition(
...
if layers:
    ...
    layer_parking_left = processing.run(
        'native:dissolve',
        {
            'INPUT': layer_parking_left,
            'FIELD': [
                'highway:name',
                'parking',
                'orientation',
    ...
    layer_parking_right = processing.run(
        'native:dissolve',
        {
            'INPUT': layer_parking_right,
            'FIELD': [
                'highway:name',
                'parking',
                'orientation',
    ...
    layer_parking_left = processing.run(
        ...
        {
            'INPUT': layer_parking_left,
            'DISTANCE': QgsProperty.fromExpression('"offset"'),
            'OUTPUT': 'memory:'
        }
    )['OUTPUT']
    layer_parking_right = processing.run(
    ...
    processing.run(
        'qgis:selectbyexpression', {'INPUT': layer_service, 'EXPRESSION': ' \"parking:lane:left\" IS NOT NULL OR \"parking:lane:right\" IS NOT NULL'})
    intersects = processing.run(
        'native:lineintersections',
        {
            'INPUT': QgsProcessingFeatureSourceDefinition(
    ...
    intersects_buffer = processing.run(
        'native:buffer',
        {
            'DISTANCE': QgsProperty.fromExpression('("width_proc_2" / 2) + 5'),
            'INPUT': intersects,
            'OUTPUT': 'memory:'
        }
    )['OUTPUT']
    ...
    layer_parking_service = processing.run(
        'native:difference',
        {
            'INPUT': QgsProcessingFeatureSourceDefinition(
                layer_parking.id(),
    ...
    centerline = processing.run(
        'native:offsetline',
        {
            'DISTANCE': QgsProperty.fromExpression('"parking:lane:left:offset" - (("parking:lane:left:offset" + abs("parking:lane:right:offset")) / 2)'),
            'INPUT': layer_street,
            'OUTPUT': 'memory:'
        }
    )['OUTPUT']
    carriageway_buffer = processing.run(
        'native:buffer',
        {
            'DISTANCE': QgsProperty.fromExpression('("width_proc" / 2) - 0.5'),
            'END_CAP_STYLE': 1,
            'INPUT': centerline,
            'OUTPUT': 'memory:'
        }
    ...
    # delete parking lane segments/line artefacts if they are too short for parking and add capacity information to line segments or correct cutting errors
    ...
    # layer_parking_chain = processing.run(
    #     'native:pointsalonglines',
    #     {
    #         'INPUT': layer_parking,
    #         'DISTANCE': QgsProperty.fromExpression(
    #             """if("orientation" = 'parallel' OR "orientation" = 'diagonal' OR "orientation" = 'perpendicular' OR "orientation" = 'marked', $length / "capacity", 0)"""
    #         ),
    #         'START_OFFSET': QgsProperty.fromExpression(
    #             """if("parking" = 'parallel', 2.6 - 0.4, if("parking" = 'diagonal', 1.27, if("parking" = 'perpendicular', 1.25 - 0.4, 0)))"""
    #         ),
    #         'END_OFFSET': QgsProperty.fromExpression(
    #             """if("parking" = 'parallel', 2.6 - 0.4, if("parking" = 'diagonal', 3.11, if("parking" = 'perpendicular', 1.25 - 0.4, 0)))"""
    #         ),
    #         'OUTPUT': 'memory:'
    #     }
    # )['OUTPUT']
    ...
    layer_parking_chain = processing.run(
        ...
        {
            'INPUT': layer_parking,
            'DISTANCE': QgsProperty.fromExpression("""if("source:capacity" = 'estimated', if("orientation" = 'diagonal', 3.1, if("orientation" = 'perpendicular', 2.5, 5.2)), if("capacity" = 1, $length, if($length < if("orientation" = 'diagonal', 3.1 * "capacity", if("orientation" = 'perpendicular', 2.5 * "capacity", (5.2 * "capacity") - 0.8)), ($length + (if("orientation" = 'parallel', 0.8, if("orientation" = 'perpendicular', 0.5, 0))) - (2 * if("orientation" = 'diagonal', 1.55, if("orientation" = 'perpendicular', 1.25, 2.6)))) / ("capacity" - 1), ($length - (2 * if("orientation" = 'diagonal', 1.55, if("orientation" = 'perpendicular', 1.25, 2.6)))) / ("capacity" - 1))))"""),  # noqa  # E501  # line too long
            'START_OFFSET': QgsProperty.fromExpression("""if("source:capacity" = 'estimated', if("orientation" = 'diagonal', ($length - (3.1*("capacity" - 1))) / 2, if("orientation" = 'perpendicular', ($length - (2.5*("capacity" - 1))) / 2, ($length - (5.2*("capacity" - 1))) / 2)), if("capacity" < 2, $length / 2, if("orientation" = 'diagonal', 1.55, if("orientation" = 'perpendicular', if($length < if("orientation" = 'diagonal', 3.1 * "capacity", if("orientation" = 'perpendicular', 2.5 * "capacity", (5.2 * "capacity") - 0.8)), 0.9, 1.25), if($length < if("orientation" = 'diagonal', 3.1 * "capacity", if("orientation" = 'perpendicular', 2.5 * "capacity", (5.2 * "capacity") - 0.8)), 2.2, 2.6)))))"""),  # noqa  # E501  # line too long
            'OUTPUT': 'memory:'
        }
    )['OUTPUT']
    layer_parking_chain = processing.run(
