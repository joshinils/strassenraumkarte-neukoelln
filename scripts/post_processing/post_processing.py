...
class VariablesAndSettings:
    # processing steps - set "1" for the needed step(s) befor running the script:
    proc_crossings       = 1     # < # Fahrbahnbezogene Eigenschaften von Querungsstellen ermitteln und übertragen                                 # noqa # TODO
    proc_cr_markings     = 1     #   # Querungsstellen mit randseitigen Markierungen am Bordstein ausrichten                                       # noqa # TODO
    proc_cr_lines        = 1     # < # Linien markierter Gehweg-Querungsstellen erzeugen                                                           # noqa # TODO
    proc_cr_tactile_pav  = 1     # < # Taktile Bodenleitsysteme entlang von Bordsteinen und Wegen generieren                                       # noqa # TODO
    proc_lane_markings   = 1     # < # Straßenmarkierungen erzeugen                                                                                # noqa # TODO
    proc_highway_backup  = 1     # < # Straßenlinien als Backup in fahrbahnfreien Bereichen erzeugen                                               # noqa # TODO
    proc_service         = 1     #   # service-Wege mit gleichen Eigenschaften zusammenführen, um Lücken zu vermeiden                              # noqa # TODO
    proc_oneways         = 1     #   # Für Einbahnstraßen separate Linien zur Markierung erzeugen                                                  # noqa # TODO
    proc_traffic_calming = 1     #   # Straßeneigenschaften auf Verkehrsberuhigungsmaßnahmen übertragen                                            # noqa # TODO
    proc_cycleways       = 1     #   # Radwege nachbearbeiten                                                                                      # noqa # TODO
    proc_path_areas      = 1     #   # Vereinigt aneinander angrenzende Wegeflächen und erzeugt deren Outlines                                     # noqa # TODO
    proc_building_parts  = 1     # < # Gebäudeteile auf Stockwerksunterschiede auflösen, Gebäudegrundrisse / schwebende Gebäudeteile verarbeiten   # noqa # TODO
    proc_housenumbers    = 1     #   # Hausnummern gleichmäßig zum Gebäudeumriss ausrichten                                                        # noqa # TODO
    proc_water_body      = 1     # < # Gewässerkörper zu einem Einzelpolygon vereinigen                                                            # noqa # TODO
    proc_landcover       = 1     #   # Bereiche mit "landcover=*" in Polygone umwandeln (werden nur als Linien erkannt)                            # noqa # TODO
    proc_playground      = 1     #   # Gras-Flächen zur besseren Darstellung aus playground-Polygonen ausstanzen                                   # noqa # TODO
    proc_orient_man_made = 1     # < # Richtet bestimmte Straßenmöbel zur nächstgelegenen Straße hin aus (Straßenlaternen, Schaltkästen)           # noqa # TODO
    proc_trees           = 1     # < # Baumkronendurchmesser und Stammumfang abschätzen                                                            # noqa # TODO
    proc_forests         = 1     #   # Waldbäume über ein hexagonales Gitter interpolieren                                                         # noqa # TODO
    proc_cars            = 1     #   # Fahrzeuge auf Parkstreifen mit Farben und Fahrzeugmodellen generieren                                       # noqa # TODO
    proc_labels          = 1     #   # bessere Segmente zur Beschriftung von Straßennamen und Gewässern generieren                                 # noqa # TODO

    # project directory
    main_dir = '/home/niels/Documents/strassenraumkarte-neukoelln/scripts/post_processing/'  # <<<<<<<< fill in your project directory here!
    data_dir = main_dir + 'layer/geojson/'
    proc_dir = main_dir + 'layer/geojson/post_processed/'

    if not os.path.exists(main_dir):
        raise ValueError(f"the value for main_dir does not exist: {main_dir}")
    pathlib.Path(data_dir).mkdir(parents=True, exist_ok=True)
    pathlib.Path(proc_dir).mkdir(parents=True, exist_ok=True)

    logging.debug(f"main_dir {main_dir}")
    logging.debug(f"data_dir {data_dir}")
    logging.debug(f"proc_dir {proc_dir}")
...
def prepareLayers(layer_street, layer_path, layer_crossing):
    ...
    Args:
        layer_street (_type_): _description_  # TODO
        layer_path (_type_): _description_  # TODO
        layer_crossing (_type_): _description_  # TODO

    Returns:
        _type_: _description_  # TODO
    """
    logging.info('   Füge Straßendaten ein...')

    # Straßen-Input in einen Straßen- und einen Einfahrtlayer teilen

    processing.run('native:reprojectlayer', {'INPUT': VariablesAndSettings.layer_raw_highway_ways, 'TARGET_CRS': QgsCoordinateReferenceSystem(VariablesAndSettings.crs_to), 'OUTPUT': VariablesAndSettings.proc_dir + 'streets_processed.geojson'})
    processing.run('native:reprojectlayer', {'INPUT': layer_crossing, 'TARGET_CRS': QgsCoordinateReferenceSystem(VariablesAndSettings.crs_to), 'OUTPUT': VariablesAndSettings.proc_dir + 'crossing.geojson'})
    layer_street = QgsProject.instance().addMapLayer(QgsVectorLayer(VariablesAndSettings.proc_dir + 'streets_processed.geojson', 'Straßen', 'ogr'), False)
    layer_crossing = QgsProject.instance().addMapLayer(QgsVectorLayer(VariablesAndSettings.proc_dir + 'crossing.geojson', 'Übergänge', 'ogr'), False)

    logging.info('   Bearbeite Daten: Datensatz bereinigen...')
    layer_street = clearAttributes(layer_street, VariablesAndSettings.street_key_list)
    ...
    for feature in layer_street.getFeatures():
        ...
        # Ansonsten Breite aus anderen Straßenattributen abschätzen
            ...
            if highway == 'primary':
                width = VariablesAndSettings.width_primary_street
            if highway == 'secondary':
                width = VariablesAndSettings.width_secondary_street
            if highway == 'tertiary':
                width = VariablesAndSettings.width_tertiary_street
            ...
            if highway in VariablesAndSettings.is_service_list:
                width = VariablesAndSettings.width_service
                if layer_street.fields().indexOf('service') != -1:
                    if feature.attribute('service') == 'driveway':
                        width = VariablesAndSettings.width_driveway
            if highway == 'construction':
                if constr:
                    ...
                    if construction in VariablesAndSettings.is_service_list:
                        width = VariablesAndSettings.width_service

            if width == NULL:
                width = VariablesAndSettings.width_minor_street
    ...
    return([layer_street, layer_path, layer_crossing])  # TODO: was machen die klammern hier? soll das ein tuple werden?
...
def clearAttributes(layer, attributes):
    ...
    Args:
        layer (_type_): _description_  # TODO
        attributes (_type_): _description_  # TODO
    ...
    layer.updateFields()
    layer.commitChanges()
    return(layer)  # TODO: was machen die klammern hier? soll das ein tuple werden?
...
def getDelimitedAttributes(attribute_string, deli_char, var_type):
    ...
    Args:
        attribute_string (_type_): _description_  # TODO
        deli_char (_type_): _description_  # TODO
        var_type (_type_): _description_  # TODO
    ...
    return(attribute_array)  # TODO: was machen die klammern hier? soll das ein tuple werden?


def offsetVertex(
    *, layer_lanes,
    layer_lanes_single_carriageway,
    lane_dual,
    geom,
    vertex,
    vertex_x,
    ...
    Args:
        layer_lanes (_type_): _description_  # TODO
        layer_lanes_single_carriageway (_type_): _description_  # TODO
        lane_dual (_type_): _description_  # TODO
        geom (_type_): _description_  # TODO
        vertex (_type_): _description_  # TODO
        vertex_x (_type_): _description_  # TODO
        vertex_y (_type_): _description_  # TODO
    ...
    angle_dual = math.degrees(geom.angleAtVertex(vertex))  # noqa  # F841 assigned but never used  # TODO
    ...
    placement_dual = lane_dual.attribute('placement_abs')
    ...
    lanes_dual = int(lane_dual.attribute('lanes'))
    ...
    if dual_carriageway_opposite_direction == 1:
        ...
        for lanes in range(1, len(width_lanes) + 1):
            if lanes >= placement_lane_dual_reverse and lanes < placement_lane_single:
                distance += float(width_lanes[lanes - 1]) / 2 + float(width_lanes[lanes]) / 2
    ...
    if dual_carriageway_opposite_direction == 0:
        for lanes in range(1, len(width_lanes) + 1):
            if lanes > placement_lane_single and lanes <= placement_lane_dual + lanes_diff:
                distance += float(width_lanes[lanes - 1])
    ...
    return([xv, yv])  # TODO: was machen die klammern hier? soll das ein tuple werden?
...
def offsetLaneTransition(layer, lane, distance, fix):
    ...
    Args:
        layer (_type_): _description_  # TODO
        lane (_type_): _description_  # TODO
        distance (_type_): _description_  # TODO
        fix (_type_): _description_  # TODO
    ...
    for vertex in range(vertex_count):
        ...
        # logging.debug(lane.attribute('id'), vertex_count, vertex, geom.distanceToVertex(vertex_count - 1), geom.distanceToVertex(vertex))
    ...
    # layer.commitChanges()
    return(True)  # TODO: was machen die klammern hier? soll das ein tuple werden?
...
def getConnectedSegments(layer, feature, vertex):
    ...
    Args:
        layer (_type_): _description_  # TODO
        feature (_type_): _description_  # TODO
        vertex (_type_): _description_  # TODO
...
def getAbsolutePlacement(lanes, lanes_backward, placement, placement_forward, placement_backward, l_extra):
    ...
    Args:
        lanes (_type_): _description_  # TODO
        lanes_backward (_type_): _description_  # TODO
        placement (_type_): _description_  # TODO
        placement_forward (_type_): _description_  # TODO
        placement_backward (_type_): _description_  # TODO
        l_extra (_type_): _description_  # TODO
    """
    ...
    l = int(placement[-1])  # noqa  # TODO: rename variable l
    l += l_f  # noqa  # TODO: rename variable l
    if l_b:
        l = l_b  # noqa  # TODO: rename variable l

    l += l_extra  # noqa  # TODO: rename variable l

    placement = placement[0:-1] + str(l)
    return(placement)  # TODO: was machen die klammern hier? soll das ein tuple werden?
...
def script():
    ...
    if VariablesAndSettings.proc_crossings:
        logging.info('Verarbeite Querungsstellen...')
        # Straßenbezogene Layer einladen und vorbereiten

        logging.info('   Bereite Straßendaten vor...')
        if not VariablesAndSettings.layer_raw_highway_ways:
            VariablesAndSettings.layer_raw_highway_ways = QgsVectorLayer(VariablesAndSettings.data_dir + 'highway.geojson|geometrytype=LineString', 'highway (raw)', 'ogr')
        if not VariablesAndSettings.layer_raw_path_ways:
            VariablesAndSettings.layer_raw_path_ways = QgsVectorLayer(VariablesAndSettings.data_dir + 'path.geojson|geometrytype=LineString', 'path (raw)', 'ogr')
        if not VariablesAndSettings.layer_raw_highway_points:
            VariablesAndSettings.layer_raw_highway_points = QgsVectorLayer(VariablesAndSettings.data_dir + 'highway.geojson|geometrytype=Point', 'highway (raw)', 'ogr')
        layers = prepareLayers(VariablesAndSettings.layer_raw_highway_ways, VariablesAndSettings.layer_raw_path_ways, VariablesAndSettings.layer_raw_highway_points)
        if layers:
            ...
            logging.info('   Eigenschaften auf Querungsstellen übertragen...')
            # Straßenbreite
            ...
            layer_crossing = processing.run('native:deleteduplicategeometries', {'INPUT': layer_crossing, 'OUTPUT': VariablesAndSettings.proc_dir + 'crossing.geojson'})

    # Querungsstellen mit randseitigen Markierungen am Bordstein ausrichten
    if VariablesAndSettings.proc_cr_markings:
        logging.info('Querungsstellen ausrichten...')
        layer_crossing = QgsVectorLayer(VariablesAndSettings.proc_dir + 'crossing.geojson|geometrytype=Point', 'Querungsstellen', 'ogr')
        ...
        if not VariablesAndSettings.layer_raw_path_ways:
            VariablesAndSettings.layer_raw_path_ways = QgsVectorLayer(VariablesAndSettings.data_dir + 'path.geojson|geometrytype=LineString', 'path (raw)', 'ogr')
        layer_crossing_ways = processing.run('native:reprojectlayer', {'INPUT': VariablesAndSettings.layer_raw_path_ways, 'TARGET_CRS': QgsCoordinateReferenceSystem(VariablesAndSettings.crs_to), 'OUTPUT': 'memory:'})['OUTPUT']
        ...
        logging.info('   Ausrichtungswinkel bestimmen...')
        ...
        logging.info('   Randseitige Markierungen versetzen...')
        ...
        logging.info('   Randseitige Markierungen snappen (1/2)...')
        if not VariablesAndSettings.layer_raw_kerb_street_areas_polygons:
            VariablesAndSettings.layer_raw_kerb_street_areas_polygons = QgsVectorLayer(VariablesAndSettings.data_dir + 'kerb/kerb_street_areas.geojson|geometrytype=Polygon', 'Fahrbahnbereiche (raw)', 'ogr')
        layer_kerb_lines = processing.run('native:polygonstolines', {'INPUT': VariablesAndSettings.layer_raw_kerb_street_areas_polygons, 'OUTPUT': 'memory:'})['OUTPUT']
        ...
        logging.info('   Randseitige Markierungen snappen (2/2)...')
        ...
        logging.info('   Randseitige Markierungen feinjustieren (1/2)...')
        ...
        layer_kerb_nodes = processing.run('qgis:extractbyexpression', {'INPUT': VariablesAndSettings.layer_raw_highway_points, 'EXPRESSION': '"kerb" IS NOT NULL', 'OUTPUT': 'memory:'})['OUTPUT']
        layer_kerb_nodes = processing.run('native:reprojectlayer', {'INPUT': layer_kerb_nodes, 'TARGET_CRS': QgsCoordinateReferenceSystem(VariablesAndSettings.crs_to), 'OUTPUT': 'memory:'})['OUTPUT']
        ...
        logging.info('   Randseitige Markierungen feinjustieren (2/2)...')
        ...
        logging.info('   Bordsteinwinkel übertragen...')
        ...
        QgsVectorFileWriter.writeAsVectorFormat(layer_crossing_buffer_markings, VariablesAndSettings.proc_dir + 'crossing_buffer_markings.geojson', 'utf-8', QgsCoordinateReferenceSystem(VariablesAndSettings.crs_from), 'GeoJson')

    # Linien markierter Gehweg-Querungsstellen erzeugen
    if VariablesAndSettings.proc_cr_lines:
        logging.info('Markierungen an Querungsstellen erzeugen...')
        logging.info('   Datensätze laden...')
        layer_crossing = QgsVectorLayer(VariablesAndSettings.proc_dir + 'crossing.geojson|geometrytype=Point', 'Querungsstellen', 'ogr')
        if not VariablesAndSettings.layer_raw_path_ways:
            VariablesAndSettings.layer_raw_path_ways = QgsVectorLayer(VariablesAndSettings.data_dir + 'path.geojson|geometrytype=LineString', 'path (raw)', 'ogr')
        layer_crossing_path = VariablesAndSettings.layer_raw_path_ways

        if not VariablesAndSettings.layer_raw_kerb_street_areas_polygons:
            VariablesAndSettings.layer_raw_kerb_street_areas_polygons = QgsVectorLayer(VariablesAndSettings.data_dir + 'kerb/kerb_street_areas.geojson|geometrytype=Polygon', 'Fahrbahnbereiche (raw)', 'ogr')
        layer_carriageway = VariablesAndSettings.layer_raw_kerb_street_areas_polygons
        ...
        layer_crossing_path = processing.run('native:reprojectlayer', {'INPUT': layer_crossing_path, 'TARGET_CRS': QgsCoordinateReferenceSystem(VariablesAndSettings.crs_to), 'OUTPUT': 'memory:'})['OUTPUT']
        ...
        logging.info('   Lineare Querungen ermitteln (Eigenschaften vereinheitlichen)...')
        ...
        logging.info('   Lineare Querungen ermitteln (Wegstücke ermitteln)...')
        ...
        for path in layer_crossing_path.getFeatures():
            ...
            # logging.debug(id, ' | draw: ', draw_lines, ' | vertex: ', index, ' | vertex_max: ', vertex_max_index)
        ...
        logging.info('   Markierungslinien aus crossing-Nodes ableiten...')
        ...
        logging.info('   Markierungslinien zuschneiden...')
        ...
        layer_kerbs = processing.run('native:reprojectlayer', {'INPUT': layer_kerbs, 'TARGET_CRS': QgsCoordinateReferenceSystem(VariablesAndSettings.crs_to), 'OUTPUT': 'memory:'})['OUTPUT']
        ...
        layer_buffer_markings = QgsVectorLayer(VariablesAndSettings.proc_dir + 'crossing_buffer_markings.geojson|geometrytype=Point', 'randseitige Markierungen', 'ogr')
        layer_buffer_markings = processing.run('native:reprojectlayer', {'INPUT': layer_buffer_markings, 'TARGET_CRS': QgsCoordinateReferenceSystem(VariablesAndSettings.crs_to), 'OUTPUT': 'memory:'})['OUTPUT']
        ...
        layer_crossing_lines = processing.run('qgis:extractbyexpression', {'INPUT': layer_crossing_lines, 'EXPRESSION': '$length > 1.25', 'OUTPUT': VariablesAndSettings.proc_dir + 'crossing_line_markings.geojson'})

    # Taktile Bodenleitsysteme entlang von Bordsteinen und Wegen generieren
    if VariablesAndSettings.proc_cr_tactile_pav:
        logging.info('Generiere taktile Bodenleitsysteme...')
        logging.info('   Filtere Bordsteinlinien mit Leitsystemen...')
        if not VariablesAndSettings.layer_raw_kerb_ways:
            ...
            if not VariablesAndSettings.layer_raw_barrier_ways:
                VariablesAndSettings.layer_raw_barrier_ways = QgsVectorLayer(VariablesAndSettings.data_dir + 'barriers.geojson|geometrytype=LineString', 'barrier (raw)', 'ogr')
            layer_kerb = processing.run('qgis:extractbyattribute', {'INPUT': VariablesAndSettings.layer_raw_barrier_ways, 'FIELD': 'barrier', 'VALUE': 'kerb', 'OUTPUT': 'memory:'})['OUTPUT']
            if not VariablesAndSettings.layer_raw_barrier_polygons:
                VariablesAndSettings.layer_raw_barrier_polygons = QgsVectorLayer(VariablesAndSettings.data_dir + 'barriers.geojson|geometrytype=Polygon', 'barrier (raw)', 'ogr')
            layer_kerb_outlines = processing.run('native:polygonstolines', {'INPUT': VariablesAndSettings.layer_raw_barrier_polygons, 'OUTPUT': 'memory:'})['OUTPUT']
            ...
            layer_kerb = processing.run('native:reprojectlayer', {'INPUT': layer_kerb, 'TARGET_CRS': QgsCoordinateReferenceSystem(VariablesAndSettings.crs_to), 'OUTPUT': 'memory:'})['OUTPUT']
        else:
            layer_kerb = VariablesAndSettings.layer_raw_kerb_ways
        layer_kerb = processing.run('native:reprojectlayer', {'INPUT': layer_kerb, 'TARGET_CRS': QgsCoordinateReferenceSystem(VariablesAndSettings.crs_to), 'OUTPUT': 'memory:'})['OUTPUT']
        ...
        layer_tactile_paving_nodes = processing.run('qgis:extractbyexpression', {'INPUT': VariablesAndSettings.layer_raw_highway_points, 'EXPRESSION': '"tactile_paving" = \'yes\' and "kerb" IS NOT NULL', 'OUTPUT': 'memory:'})['OUTPUT']
        layer_tactile_paving_nodes = processing.run('native:reprojectlayer', {'INPUT': layer_tactile_paving_nodes, 'TARGET_CRS': QgsCoordinateReferenceSystem(VariablesAndSettings.crs_to), 'OUTPUT': 'memory:'})['OUTPUT']

        logging.info('   Übertrage Wegebreite...')
        # Wege einladen (für Breite der Markierung und für Wegesegmente mit bordstein-unabhängigem Bodenleitsystem)
        if not VariablesAndSettings.layer_raw_path_ways:
            VariablesAndSettings.layer_raw_path_ways = QgsVectorLayer(VariablesAndSettings.data_dir + 'path.geojson|geometrytype=LineString', 'path (raw)', 'ogr')

        layer_ways = processing.run('native:reprojectlayer', {'INPUT': VariablesAndSettings.layer_raw_path_ways, 'TARGET_CRS': QgsCoordinateReferenceSystem(VariablesAndSettings.crs_to), 'OUTPUT': 'memory:'})['OUTPUT']
        ...
        logging.info('   Integriere Wegesegmente mit Leitsystemen...')

        # Wege mit separatem, bordstein-unabhängigem Bodenleitsystem herausfiltern
        layer_tactile_paving_ways = processing.run('qgis:extractbyexpression', {'INPUT': VariablesAndSettings.layer_raw_path_ways, 'EXPRESSION': '"tactile_paving" = \'yes\' and "footway" IS NOT \'crossing\'', 'OUTPUT': 'memory:'})['OUTPUT']
        ...
        QgsVectorFileWriter.writeAsVectorFormat(layer_tactile_paving, VariablesAndSettings.proc_dir + 'tactile_paving.geojson', 'utf-8', QgsCoordinateReferenceSystem(VariablesAndSettings.crs_from), 'GeoJson')

    # Straßenmarkierungen erzeugen
    if VariablesAndSettings.proc_lane_markings:
        logging.info('Erzeuge Straßenmarkierungen...')

        # Kreuzungsflächen laden, um dort später Straßenmarkierungen zu entfernen
        logging.info('   Lade Knotenpunktbereiche...')

        if not VariablesAndSettings.layer_raw_area_highway_polygons:
            VariablesAndSettings.layer_raw_area_highway_polygons = QgsVectorLayer(VariablesAndSettings.data_dir + 'area_highway.geojson|geometrytype=Polygon', 'area_highway (raw)', 'ogr')
        layer_junction_areas = VariablesAndSettings.layer_raw_area_highway_polygons
        layer_junction_areas = processing.run(
            'qgis:extractbyexpression',
            {
                'INPUT': layer_junction_areas,
                'EXPRESSION': """"junction" = 'yes' OR "crossing" = 'traffic_signals' OR "crossing" = 'marked' OR "crossing" = 'zebra'""",
                'OUTPUT': 'memory:'
            }
        )['OUTPUT']
        ...
        if not VariablesAndSettings.layer_raw_highway_points:
            VariablesAndSettings.layer_raw_highway_points = QgsVectorLayer(VariablesAndSettings.data_dir + 'highway.geojson|geometrytype=Point', 'highway (raw)', 'ogr')
        layer_stop_nodes = processing.run('qgis:extractbyexpression', {'INPUT': VariablesAndSettings.layer_raw_highway_points, 'EXPRESSION': '"highway" = \'traffic_signals\' or "highway" = \'stop\'', 'OUTPUT': 'memory:'})['OUTPUT']
        ...
        logging.info('   Lade Straßennetz...')
        if not VariablesAndSettings.layer_raw_highway_ways:
            VariablesAndSettings.layer_raw_highway_ways = QgsVectorLayer(VariablesAndSettings.data_dir + 'highway.geojson|geometrytype=LineString', 'highway (raw)', 'ogr')
        layer_lanes = VariablesAndSettings.layer_raw_highway_ways
        ...
        logging.info('   Vereinfache Straßeninformationen...')
        ...
        layer_lanes = clearAttributes(layer_lanes, VariablesAndSettings.lanes_attributes)
        # in metrisches Koordinatensystem konvertieren
        layer_lanes = processing.run('native:reprojectlayer', {'INPUT': layer_lanes, 'TARGET_CRS': QgsCoordinateReferenceSystem(VariablesAndSettings.crs_to), 'OUTPUT': 'memory:'})['OUTPUT']

        # QgsProject.instance().addMapLayer(layer_lanes, True)

        logging.info('   Bereite Fahrspuren vor...')
        ...
        for lane in layer_lanes.getFeatures():
            ...
            # logging.debug(lane.attribute('id')) # Anzeigen, um fehlerhafte Segmente in den OSM-Daten zu finden, die Script-Error produzieren
            ...
            if layer_lanes.fields().indexOf('dual_carriageway') != -1:
                dual_carriageway = lane.attribute('dual_carriageway')  # noqa  # F841 assigned but never used  # TODO
            ...
            if layer_lanes.fields().indexOf('bus:lanes:forward') != -1:
                bus_lanes_forward = lane.attribute('bus:lanes:forward')  # noqa  # F841 assigned but never used  # TODO
            if layer_lanes.fields().indexOf('bus:lanes:backward') != -1:
                bus_lanes_backward = lane.attribute('bus:lanes:backward')  # noqa  # F841 assigned but never used  # TODO
            ...
            if layer_lanes.fields().indexOf('psv:lanes:forward') != -1:
                psv_lanes_forward = lane.attribute('psv:lanes:forward')  # noqa  # F841 assigned but never used  # TODO
            if layer_lanes.fields().indexOf('psv:lanes:backward') != -1:
                psv_lanes_backward = lane.attribute('psv:lanes:backward')  # noqa  # F841 assigned but never used  # TODO
            ...
            if not cycleway_right_width:
                if cycleway_width:
                    ...
                    cycleway_right_width = VariablesAndSettings.cycleway_width_default
            if not cycleway_left_width:
                if cycleway_width:
                    ...
                    cycleway_left_width = VariablesAndSettings.cycleway_width_default
            ...
            if width_lanes:
                ...
                if len(width_lanes) < lanes:
                    for i in range(lanes - len(width_lanes)):
                        width_lanes.append(VariablesAndSettings.lane_width_default)
                # leere Spurbreiten mit Default ersetzen
                if '' in width_lanes:
                    for i in range(len(width_lanes)):
                        if width_lanes[i] == '':
                            width_lanes[i] = VariablesAndSettings.lane_width_default
            else:
            ...
            # if lane.attribute('id') == 'way/964618340':
                # logging.debug(lanes, lanes_forward, lanes_backward, ' | ', lanes_marked, lanes_forward_marked, lanes_backward_marked, ' | ', lanes_unmarked, lanes_forward_unmarked, lanes_backward_unmarked)
            ...
            if((cycleway == 'lane' or cycleway_both == 'lane' or cycleway_right == 'lane' or cycleway_left == 'lane') and cycleway_lanes.count('lane') == 0):  # Radfahrstreifen in Mittellage werden bereits separat ausgewertet
                ...
                if cyclelanes_backward:
                    l = int(placement[-1])  # noqa   # TODO rename variable l
                    l += 1  # noqa   # TODO rename variable l
                    placement = placement[0:-1] + str(l)
                    layer_lanes.changeAttributeValue(lane.id(), layer_lanes.fields().indexOf('placement_abs'), placement)
            ...
            for l in range(lanes):  # noqa   # TODO rename variable l
                if l < reverse_count:
        ...
        if layer_lanes.fields().indexOf('dual_carriageway') != -1:
            logging.info('   Korrigiere Verzweigungspunkte...')
            ...
            for lane in layer_lanes.getFeatures():
                ...
                if move_start_vertex == 1:
                    offset = offsetVertex(
                        ...
                        layer_lanes_single_carriageway=layer_lanes_single_carriageway,
                        lane_dual=lane,
                        geom=geom,
                        vertex=0,
                        vertex_x=start_vertex_x,
                        vertex_y=start_vertex_y,
                        lanes_dict=lanes_dict
                    )
                    xv = offset[0]
                    yv = offset[1]
                ...
                if move_end_vertex == 1:
                    offset = offsetVertex(
                        ...
                        vertex_x=end_vertex_x,
                        vertex_y=end_vertex_y,
                        lanes_dict=lanes_dict
                    )
                    xv = offset[0]
                    yv = offset[1]
        ...
        logging.info('   Korrigiere placement transition...')
        # layer_lanes.startEditing()
        for lane in layer_lanes.getFeatures():
            ...
            if id_lane in transition_dict:
                ...
                if not placement_to:
                    if len(lanes_dict['segments_after'][id_lane]['id']):
                        ...
                        logging.debug('placement-transition-Prozess übersprungen - kein Anschlusssegment: ', id_lane)
                        continue
                ...
                placement_lane_from = placement_lane_from_value = int(placement_from[-1])  # noqa  # F841 assigned but never used  # TODO
                placement_lane_to = placement_lane_to_value = int(placement_to[-1])  # noqa  # F841 assigned but never used  # TODO
        ...
        logging.info('   Erzeuge Fahrspuren...')
        layer_lanes = processing.run('native:arrayoffsetlines', {'INPUT': layer_lanes, 'COUNT': QgsProperty.fromExpression('"lanes" - 1'), 'OFFSET': - VariablesAndSettings.lane_width_default, 'JOIN_STYLE': 1, 'OUTPUT': 'memory:'})['OUTPUT']
        ...
        for lane in layer_lanes.getFeatures():
            ...
            i = 0
            if lanes > 1:
                width_default_offset = VariablesAndSettings.lane_width_default - float(width_lanes[0])
                for i in range(instance):
                    if i == 0:
                        width_default += VariablesAndSettings.lane_width_default / 2
                        width_default += width_default_offset / 2
                        width_sum += float(width_lanes[i]) / 2
                        buffer += float(buffer_left_lanes[i])
                    else:
                        width_default += VariablesAndSettings.lane_width_default
            ...
            if turn == 'slide_right':
                turn = 'merge_to_right'
            if turn not in ['through', 'left', 'right', 'left;through', 'through;right', 'merge_to_left', 'merge_to_right']:
                turn = 'none'
        ...
        logging.info('   Bereite Fahrspuren auf...')
        ...
        if not VariablesAndSettings.layer_raw_path_ways:
            VariablesAndSettings.layer_raw_path_ways = QgsVectorLayer(VariablesAndSettings.data_dir + 'path.geojson|geometrytype=LineString', 'path (raw)', 'ogr')
        layer_crossings = processing.run('qgis:extractbyexpression', {'INPUT': VariablesAndSettings.layer_raw_path_ways, 'EXPRESSION': '"footway" = \'crossing\' and ("crossing" = \'traffic_signals\' or "crossing" = \'marked\' or "crossing" = \'zebra\')', 'OUTPUT': 'memory:'})['OUTPUT']
        layer_crossings = processing.run('native:reprojectlayer', {'INPUT': layer_crossings, 'TARGET_CRS': QgsCoordinateReferenceSystem(VariablesAndSettings.crs_to), 'OUTPUT': 'memory:'})['OUTPUT']
        ...
        layer_prohibited = processing.run('qgis:extractbyexpression', {'INPUT': VariablesAndSettings.layer_raw_area_highway_polygons, 'EXPRESSION': '"area:highway" = \'prohibited\'', 'OUTPUT': 'memory:'})['OUTPUT']
        layer_lanes.startEditing()
        ...
        QgsVectorFileWriter.writeAsVectorFormat(layer_lanes, VariablesAndSettings.proc_dir + 'marked_lanes.geojson', 'utf-8', QgsCoordinateReferenceSystem(VariablesAndSettings.crs_from), 'GeoJson')

        # Abschließend Haltelinien generieren
        logging.info('   Generiere Haltelinien...')
        lanes_attributes_plus_id = VariablesAndSettings.lanes_attributes
        lanes_attributes_plus_id.insert(0, 'id')
        layer_stop_nodes = processing.run('native:joinattributesbylocation', {'INPUT': layer_stop_nodes, 'JOIN': VariablesAndSettings.layer_raw_highway_ways, 'JOIN_FIELDS': lanes_attributes_plus_id, 'METHOD': 1, 'PREDICATE': [0], 'PREFIX': 'highway:', 'OUTPUT': 'memory:'})['OUTPUT']

        # Haltelinienpunkte und Knotenpunktbereiche in metrische KBS reprojizieren
        layer_stop_nodes = processing.run('native:reprojectlayer', {'INPUT': layer_stop_nodes, 'TARGET_CRS': QgsCoordinateReferenceSystem(VariablesAndSettings.crs_to), 'OUTPUT': 'memory:'})['OUTPUT']
        ...
        layer_junction_areas_outlines = processing.run('native:reprojectlayer', {'INPUT': layer_junction_areas_outlines, 'TARGET_CRS': QgsCoordinateReferenceSystem(VariablesAndSettings.crs_to), 'OUTPUT': 'memory:'})['OUTPUT']
        ...
        if not VariablesAndSettings.layer_raw_landuse_polygons:
            VariablesAndSettings.layer_raw_landuse_polygons = QgsVectorLayer(VariablesAndSettings.data_dir + 'landuse.geojson|geometrytype=Polygon', 'landuse polygons (raw)', 'ogr')
        layer_landuse_polygons = processing.run('native:reprojectlayer', {'INPUT': VariablesAndSettings.layer_raw_landuse_polygons, 'TARGET_CRS': QgsCoordinateReferenceSystem(VariablesAndSettings.crs_to), 'OUTPUT': 'memory:'})['OUTPUT']
        ...
        layer_stop_lines = QgsVectorLayer("LineString?crs=" + VariablesAndSettings.crs_to + "&field=road_marking:string&field=road_marking:left:string&field=road_marking:right:string&field=stop_line:string", "Haltelinien", "memory")
        ...
        for stop_node in layer_stop_nodes.getFeatures():
            ...
            if highway_id not in lanes_dict['placement']:
                continue
            ...
            if not len(highway_line):
                # Falls keine Straße durch die Ampel führt
                continue
            ...
            if not line_from_junction:
                ...
                # logging.debug(highway_id, direction, round(angle, 2), round(len_empty, 2), round(len_left, 2), round(len_right, 2))
                # logging.debug(highway_id, len_empty, len_left, len_right, ' | ', start_x, start_y, end_x, end_y)
            ...
            # falls vorhanden, Haltelinien aus Knotenpunkt-Geometrien ableiten
                ...
                if not ow:
                    ...
                    layer_split_line = QgsVectorLayer("LineString?crs=" + VariablesAndSettings.crs_to, "split_line", "memory")
        ...
        layer_road_markings_raw = QgsVectorLayer(VariablesAndSettings.data_dir + 'area_highway.geojson|geometrytype=LineString', 'area_highway (raw)', 'ogr')
        ...
        layer_road_markings = processing.run('native:clip', {'INPUT': QgsProcessingFeatureSourceDefinition(layer_road_markings.id(), flags=QgsProcessingFeatureSourceDefinition.FlagOverrideDefaultGeometryCheck, geometryCheck=QgsFeatureRequest.GeometrySkipInvalid), 'OVERLAY': VariablesAndSettings.layer_raw_kerb_street_areas_polygons, 'OUTPUT': 'memory:'})['OUTPUT']
        # sehr kurze Segmente (Relikte bis 20 cm) entfernen
        layer_road_markings = processing.run('qgis:extractbyexpression', {'INPUT': layer_road_markings, 'EXPRESSION': '$length > 0.2', 'OUTPUT': VariablesAndSettings.proc_dir + 'road_markings.geojson'})

    # Straßenlinien als Backup in fahrbahnfreien Bereichen erzeugen
    if VariablesAndSettings.proc_highway_backup:
        logging.info('Erzeuge Backup-Straßennetz...')
        if not VariablesAndSettings.layer_raw_highway_ways:
            VariablesAndSettings.layer_raw_highway_ways = QgsVectorLayer(VariablesAndSettings.data_dir + 'highway.geojson|geometrytype=LineString', 'highway (raw)', 'ogr')
        layer_highway = VariablesAndSettings.layer_raw_highway_ways
        ...
        if not VariablesAndSettings.layer_raw_kerb_street_areas_polygons:
            VariablesAndSettings.layer_raw_kerb_street_areas_polygons = QgsVectorLayer(VariablesAndSettings.data_dir + 'kerb/kerb_street_areas.geojson|geometrytype=Polygon', 'Fahrbahnbereiche (raw)', 'ogr')
        layer_kerbs = VariablesAndSettings.layer_raw_kerb_street_areas_polygons
        ...
        layer_highway = processing.run('native:difference', {'INPUT': layer_highway, 'OVERLAY': layer_highway_clipped, 'OUTPUT': VariablesAndSettings.proc_dir + 'highway_backup.geojson'})

    # service-Wege mit gleichen Eigenschaften zusammenführen, um Lücken zu vermeiden
    if VariablesAndSettings.proc_service:
        logging.info('Vereinige service-Straßen...')
        if not VariablesAndSettings.layer_raw_highway_ways:
            VariablesAndSettings.layer_raw_highway_ways = QgsVectorLayer(VariablesAndSettings.data_dir + 'highway.geojson|geometrytype=LineString', 'highway (raw)', 'ogr')
        layer_highway = VariablesAndSettings.layer_raw_highway_ways
        ...
        layer_highway = processing.run('native:multiparttosingleparts', {'INPUT': layer_highway, 'OUTPUT': VariablesAndSettings.proc_dir + 'service.geojson'})

    # Für Einbahnstraßen separate Linien erzeugen
    if VariablesAndSettings.proc_oneways:
        logging.info('Extrahiere Einbahnstraßen...')
        logging.info('   Lade Wegedaten...')
        if not VariablesAndSettings.layer_raw_highway_ways:
            VariablesAndSettings.layer_raw_highway_ways = QgsVectorLayer(VariablesAndSettings.data_dir + 'highway.geojson|geometrytype=LineString', 'highway (raw)', 'ogr')
        if not VariablesAndSettings.layer_raw_path_ways:
            VariablesAndSettings.layer_raw_path_ways = QgsVectorLayer(VariablesAndSettings.data_dir + 'path.geojson|geometrytype=LineString', 'path (raw)', 'ogr')
        ...
        logging.info('   Filtere Einbahnstraßen...')
        VariablesAndSettings.layer_raw_highway_ways = processing.run('qgis:extractbyexpression', {'INPUT': VariablesAndSettings.layer_raw_highway_ways, 'EXPRESSION': '\"oneway\" = \'yes\' OR \"oneway\" = \'-1\'', 'OUTPUT': 'memory:'})['OUTPUT']
        VariablesAndSettings.layer_raw_path_ways = processing.run('qgis:extractbyexpression', {'INPUT': VariablesAndSettings.layer_raw_path_ways, 'EXPRESSION': '\"oneway\" = \'yes\' OR \"oneway\" = \'-1\'', 'OUTPUT': 'memory:'})['OUTPUT']
        logging.info('   Führe Wegedaten zusammen...')
        layer_ways = processing.run('native:mergevectorlayers', {'LAYERS': [VariablesAndSettings.layer_raw_highway_ways, VariablesAndSettings.layer_raw_path_ways], 'OUTPUT': 'memory:'})['OUTPUT']

        # In metrisches Koordinatensystem umwandeln, um Puffer in Metern erzeugen zu können
        logging.info('   Transformiere Koordinatensystem...')
        QgsVectorFileWriter.writeAsVectorFormatV2(layer_ways, VariablesAndSettings.proc_dir + 'oneways.geojson', VariablesAndSettings.transform_context, VariablesAndSettings.save_options)
        layer_ways = QgsVectorLayer(VariablesAndSettings.proc_dir + 'oneways.geojson|geometrytype=LineString', 'Einbahnstraßen', 'ogr')

        # Zusammenhängende Segmente (Straßenzüge) verbinden
        logging.info('   Füge Wege zusammen...')
        ...
        logging.info('   Erzeuge Kreuzungspunkte (alle Wege)...')
        ...
        logging.info('   Erzeuge Kreuzungspunkte (Verkehrsstraßen)...')
        ...
        logging.info('   Kürze Verkehrsstraßen...')
        ...
        logging.info('   Kürze sonstige Wege...')
        ...
        logging.info('   Führe Daten zusammen...')
        ...
        logging.info('   Schließe Knotenpunktbereiche aus...')
        oneways = processing.run('native:difference', {'INPUT': oneways, 'OVERLAY': layer_junction_areas, 'OUTPUT': VariablesAndSettings.proc_dir + 'oneways.geojson'})

    # Straßeneigenschaften auf Verkehrsberuhigungsmaßnahmen übertragen
    if VariablesAndSettings.proc_traffic_calming:
        logging.info('Verkehrsberuhigung verarbeiten...')
        logging.info('   Straßen und Wege einladen...')
        if not VariablesAndSettings.layer_raw_highway_ways:
            VariablesAndSettings.layer_raw_highway_ways = QgsVectorLayer(VariablesAndSettings.data_dir + 'highway.geojson|geometrytype=LineString', 'highway (raw)', 'ogr')
        if not VariablesAndSettings.layer_raw_path_ways:
            VariablesAndSettings.layer_raw_path_ways = QgsVectorLayer(VariablesAndSettings.data_dir + 'path.geojson|geometrytype=LineString', 'path (raw)', 'ogr')
        if not VariablesAndSettings.layer_raw_highway_points:
            VariablesAndSettings.layer_raw_highway_points = QgsVectorLayer(VariablesAndSettings.data_dir + 'highway.geojson|geometrytype=Point', 'highway (raw)', 'ogr')
        layer_traffic_calming = processing.run('qgis:extractbyexpression', {'INPUT': VariablesAndSettings.layer_raw_highway_points, 'EXPRESSION': '"traffic_calming" IS NOT NULL', 'OUTPUT': 'memory:'})['OUTPUT']

        logging.info('   Eigenschaften übertragen...')
        # Straßen- und Wegesegmente auswählen, auf denen Verkehrsberuhigungen liegen
        layer_street_segments = processing.run('native:extractbylocation', {'INPUT': VariablesAndSettings.layer_raw_highway_ways, 'INTERSECT': layer_traffic_calming, 'PREDICATE': [0], 'OUTPUT': 'memory:'})['OUTPUT']
        layer_way_segments = processing.run('native:extractbylocation', {'INPUT': VariablesAndSettings.layer_raw_path_ways, 'INTERSECT': layer_traffic_calming, 'PREDICATE': [0], 'OUTPUT': 'memory:'})['OUTPUT']
        ...
        layer_street_segments = processing.run('native:reprojectlayer', {'INPUT': layer_street_segments, 'TARGET_CRS': QgsCoordinateReferenceSystem(VariablesAndSettings.crs_to), 'OUTPUT': 'memory:'})['OUTPUT']
        ...
        prefix = 'highway:'
        layer_traffic_calming = processing.run('native:reprojectlayer', {'INPUT': layer_traffic_calming, 'TARGET_CRS': QgsCoordinateReferenceSystem(VariablesAndSettings.crs_to), 'OUTPUT': 'memory:'})['OUTPUT']
        ...
        QgsVectorFileWriter.writeAsVectorFormat(layer_traffic_calming, VariablesAndSettings.proc_dir + 'traffic_calming.geojson', 'utf-8', QgsCoordinateReferenceSystem(VariablesAndSettings.crs_from), 'GeoJson')

    # Radwege nachbearbeiten
    if VariablesAndSettings.proc_cycleways:
        logging.info('Radwege nachbearbeiten...')
        if not VariablesAndSettings.layer_raw_path_ways:
            VariablesAndSettings.layer_raw_path_ways = QgsVectorLayer(VariablesAndSettings.data_dir + 'path.geojson|geometrytype=LineString', 'path (raw)', 'ogr')
        # Radwege und markierte Querungsstellen herausfiltern und reprojizieren
        layer_cycleways_on_lane = processing.run('qgis:extractbyexpression', {'INPUT': VariablesAndSettings.layer_raw_path_ways, 'EXPRESSION': '"highway" = \'cycleway\' and ("cycleway:type" = \'lane\' or "cycleway:type" = \'crossing\')', 'OUTPUT': 'memory:'})['OUTPUT']
        layer_cycleways_off_lane = processing.run('qgis:extractbyexpression', {'INPUT': VariablesAndSettings.layer_raw_path_ways, 'EXPRESSION': '"highway" = \'cycleway\' and "cycleway:type" IS NOT \'lane\' and "cycleway:type" IS NOT \'crossing\'', 'OUTPUT': 'memory:'})['OUTPUT']
        layer_crossings = processing.run('qgis:extractbyexpression', {'INPUT': VariablesAndSettings.layer_raw_path_ways, 'EXPRESSION': '"footway" = \'crossing\' and ("crossing" = \'traffic_signals\' or "crossing" = \'marked\' or "crossing" = \'zebra\')', 'OUTPUT': 'memory:'})['OUTPUT']
        layer_crossings = processing.run('native:reprojectlayer', {'INPUT': layer_crossings, 'TARGET_CRS': QgsCoordinateReferenceSystem(VariablesAndSettings.crs_to), 'OUTPUT': 'memory:'})['OUTPUT']
        # Radwege an markierten Querungsstellen unterbrechen
        logging.info('   Radwege an Querungsstellen unterbrechen...')
        layer_crossings_buffer = processing.run('native:buffer', {'INPUT': layer_crossings, 'DISTANCE': QgsProperty.fromExpression('if("width", "width" / 2, 3)'), 'OUTPUT': 'memory:'})['OUTPUT']
        ...
        layer_stop_lines = QgsVectorLayer(VariablesAndSettings.proc_dir + 'road_markings.geojson|geometrytype=LineString', 'road markings', 'ogr')
        ...
        logging.info('   Deckende Radwegflächen in Kreuzungsbereichen erzeugen...')
        ...
        layer_cycleways_crossings = processing.run('native:reprojectlayer', {'INPUT': layer_cycleways_crossings, 'TARGET_CRS': QgsCoordinateReferenceSystem(VariablesAndSettings.crs_to), 'OUTPUT': 'memory:'})['OUTPUT']
        ...
        if not VariablesAndSettings.layer_raw_kerb_street_areas_polygons:
            VariablesAndSettings.layer_raw_kerb_street_areas_polygons = QgsVectorLayer(VariablesAndSettings.data_dir + 'kerb/kerb_street_areas.geojson|geometrytype=Polygon', 'Fahrbahnbereiche (raw)', 'ogr')
        layer_cycleways_crossings = processing.run('native:clip', {'INPUT': layer_cycleways_crossings, 'OVERLAY': VariablesAndSettings.layer_raw_kerb_street_areas_polygons, 'OUTPUT': VariablesAndSettings.proc_dir + 'cycleways_crossing.geojson'})
        ...
        QgsVectorFileWriter.writeAsVectorFormat(layer_cycleways, VariablesAndSettings.proc_dir + 'cycleways.geojson', 'utf-8', QgsCoordinateReferenceSystem(VariablesAndSettings.crs_from), 'GeoJson')

    # Vereinigt aneinander angrenzende Wegeflächen und erzeugt deren Outlines
    if VariablesAndSettings.proc_path_areas:
        logging.info('Verarbeite Wegeflächen...')
        layer_path_areas_raw = QgsVectorLayer(VariablesAndSettings.data_dir + 'path.geojson|geometrytype=Polygon', 'Wegeflächen', 'ogr')
        ...
        layer_path_areas = processing.run('native:polygonstolines', {'INPUT': layer_path_areas, 'OUTPUT': VariablesAndSettings.proc_dir + 'path_areas_outlines.geojson'})

    # Gebäudeteile auf Stockwerksunterschiede auflösen, Gebäudegrundrisse / schwebende Gebäudeteile verarbeiten
    if VariablesAndSettings.proc_building_parts:
        logging.info('Verarbeite Gebäudeteile...')
        # Gebäudeteile mit gemeinsamen Höhenmerkmalen zusammenführen
        layer_building_parts_raw = QgsVectorLayer(VariablesAndSettings.data_dir + 'building_part.geojson|geometrytype=Polygon', 'Gebäudeteile (roh)', 'ogr')
        layer_building_parts_raw = clearAttributes(layer_building_parts_raw, VariablesAndSettings.building_key_list)
        layer_building_parts = processing.run('native:fixgeometries', {'INPUT': layer_building_parts_raw, 'OUTPUT': 'memory:'})['OUTPUT']
        layer_building_parts = processing.run('native:dissolve', {'FIELD': ['building:levels', 'building:min_level', 'roof:levels'], 'INPUT': layer_building_parts, 'OUTPUT': VariablesAndSettings.proc_dir + 'building_parts_dissolved.geojson'})

        logging.info('Erzeuge Gebäudegrundflächen...')
        # Gebäudeteile mit "building:min_level" > 0 oder "min_height" > 0 oder "roof" von Grundfläche abziehen
        if not VariablesAndSettings.layer_raw_buildings_polygons:
            VariablesAndSettings.layer_raw_buildings_polygons = QgsVectorLayer(VariablesAndSettings.data_dir + 'buildings.geojson|geometrytype=Polygon', 'buildings (raw)', 'ogr')
        layer_buildings = VariablesAndSettings.layer_raw_buildings_polygons
        layer_buildings = clearAttributes(layer_buildings, VariablesAndSettings.building_key_list)
        ...
        layer_buildings = processing.run('native:difference', {'INPUT': layer_buildings, 'OVERLAY': layer_building_parts_min_level, 'OUTPUT': VariablesAndSettings.proc_dir + 'building_parts_min_level.geojson'})

    # Hausnummern an inneren Gebäudeumring versetzen
    if VariablesAndSettings.proc_housenumbers:
        logging.info('Verarbeite Hausnummern...')
        # Gebäude und Hausnummern neu in metrischer Projektion abspeichern
        logging.info('   Wandle Daten in metrische Projektion...')
        if not VariablesAndSettings.layer_raw_buildings_polygons:
            VariablesAndSettings.layer_raw_buildings_polygons = QgsVectorLayer(VariablesAndSettings.data_dir + 'buildings.geojson|geometrytype=Polygon', 'buildings (raw)', 'ogr')
        layer_buildings = VariablesAndSettings.layer_raw_buildings_polygons
        layer_buildings = clearAttributes(layer_buildings, VariablesAndSettings.building_key_list)
        layer_housenumbers = QgsVectorLayer(VariablesAndSettings.data_dir + 'housenumber.geojson|geometrytype=Point', 'Hausnummern (Punktdaten, roh)', 'ogr')
        layer_housenumbers = clearAttributes(layer_housenumbers, ['id', 'addr:housenumber', 'addr:postcode', 'addr:street', 'addr:suburb'])

        QgsVectorFileWriter.writeAsVectorFormatV2(layer_buildings, VariablesAndSettings.proc_dir + 'buildings.geojson', VariablesAndSettings.transform_context, VariablesAndSettings.save_options)
        QgsVectorFileWriter.writeAsVectorFormatV2(layer_housenumbers, VariablesAndSettings.proc_dir + 'housenumber.geojson', VariablesAndSettings.transform_context, VariablesAndSettings.save_options)
        layer_buildings = QgsVectorLayer(VariablesAndSettings.proc_dir + 'buildings.geojson|geometrytype=Polygon', 'Gebäude', 'ogr')
        layer_housenumbers = QgsVectorLayer(VariablesAndSettings.proc_dir + 'housenumber.geojson|geometrytype=Point', 'Hausnummern (Punktdaten)', 'ogr')

        # Puffer um Gebäudegrenzen ziehen
        logging.info('   Erzeuge Gebäudepuffer...')
        ...
        logging.info('   Richte Hausnummern aus...')
        layer_housenumbers = processing.run('native:snapgeometries', {'BEHAVIOR': 1, 'INPUT': layer_housenumbers, 'REFERENCE_LAYER': layer_buildings_shrinked, 'TOLERANCE': 2.05, 'OUTPUT': VariablesAndSettings.proc_dir + 'housenumber_snapped.geojson'})

    # Gewässerkörper zu einem Einzelpolygon vereinigen
    if VariablesAndSettings.proc_water_body:
        logging.info('Vereinige Wasserkörper...')
        if not VariablesAndSettings.layer_raw_natural_polygons:
            VariablesAndSettings.layer_raw_natural_polygons = QgsVectorLayer(VariablesAndSettings.data_dir + 'natural.geojson|geometrytype=Polygon', 'natural (polygon)', 'ogr')
        layer_water = processing.run('qgis:extractbyexpression', {'INPUT': VariablesAndSettings.layer_raw_natural_polygons, 'EXPRESSION': '\"natural\" = \'water\' OR \"natural\" = \'wetland\'', 'OUTPUT': 'memory:'})['OUTPUT']
        layer_water = clearAttributes(layer_water, ['natural'])
        processing.run('native:dissolve', {'INPUT': layer_water, 'OUTPUT': VariablesAndSettings.proc_dir + 'water_body.geojson'})

    # Bereiche mit "landcover=*" in Polygone umwandeln (werden nur als Linien erkannt)
    if VariablesAndSettings.proc_landcover:
        logging.info('Wandel landcover-Objekte in Polygone um...')
        layer_landcover_lines = QgsVectorLayer(VariablesAndSettings.data_dir + 'landuse.geojson|geometrytype=LineString', 'landuse lines (raw)', 'ogr')
        QgsProject.instance().addMapLayer(layer_landcover_lines, False)
        if not VariablesAndSettings.layer_raw_landuse_polygons:
            VariablesAndSettings.layer_raw_landuse_polygons = QgsVectorLayer(VariablesAndSettings.data_dir + 'landuse.geojson|geometrytype=Polygon', 'landuse polygons (raw)', 'ogr')
        layer_landcover_polygons = VariablesAndSettings.layer_raw_landuse_polygons
        ...
        QgsVectorFileWriter.writeAsVectorFormatV2(landcover, VariablesAndSettings.proc_dir + 'landcover.geojson', VariablesAndSettings.transform_context, VariablesAndSettings.save_options)

    # Gras-Flächen zur besseren Darstellung aus playground-Polygonen ausstanzen
    if VariablesAndSettings.proc_playground:
        logging.info('Stanze Gras-Flächen aus Spielplatzflächen aus...')
        if not VariablesAndSettings.layer_raw_landuse_polygons:
            VariablesAndSettings.layer_raw_landuse_polygons = QgsVectorLayer(VariablesAndSettings.data_dir + 'landuse.geojson|geometrytype=Polygon', 'landuse polgons (raw)', 'ogr')
        layer_grass = processing.run('qgis:extractbyexpression', {'INPUT': VariablesAndSettings.layer_raw_landuse_polygons, 'EXPRESSION': '"landuse" = \'grass\'', 'OUTPUT': 'memory:'})['OUTPUT']
        if not VariablesAndSettings.layer_raw_leisure_polygons:
            VariablesAndSettings.layer_raw_leisure_polygons = QgsVectorLayer(VariablesAndSettings.data_dir + 'leisure.geojson|geometrytype=Polygon', 'leisure polgons (raw)', 'ogr')
        layer_playgrounds = processing.run('qgis:extractbyexpression', {'INPUT': VariablesAndSettings.layer_raw_leisure_polygons, 'EXPRESSION': '"leisure" = \'playground\'', 'OUTPUT': 'memory:'})['OUTPUT']
        ...
        layer_playgrounds = processing.run('native:difference', {'INPUT': layer_playgrounds, 'OVERLAY': layer_grass_overlay, 'OUTPUT': VariablesAndSettings.proc_dir + 'playgrounds_clipped.geojson'})

    # Richtet bestimmte Straßenmöbel zur nächstgelegenen Straße hin aus (Straßenlaternen, Schaltkästen)
    if VariablesAndSettings.proc_orient_man_made:
        logging.info('Straßenmöbel ausrichten...')
        logging.info('   Straßenreferenz einladen...')
        if not VariablesAndSettings.layer_raw_highway_ways:
            VariablesAndSettings.layer_raw_highway_ways = QgsVectorLayer(VariablesAndSettings.data_dir + 'highway.geojson|geometrytype=LineString', 'highway (raw)', 'ogr')
        if not VariablesAndSettings.layer_raw_path_ways:
            VariablesAndSettings.layer_raw_path_ways = QgsVectorLayer(VariablesAndSettings.data_dir + 'path.geojson|geometrytype=LineString', 'path (raw)', 'ogr')
        logging.info('   Straßenreferenz zusammenführen...')
        layer_full_way_network = processing.run('native:mergevectorlayers', {'CRS': QgsCoordinateReferenceSystem(VariablesAndSettings.crs_to), 'LAYERS': [VariablesAndSettings.layer_raw_highway_ways, VariablesAndSettings.layer_raw_path_ways], 'OUTPUT': 'memory:'})['OUTPUT']
        # # layer_full_way_network = clearAttributes(layer_full_way_network, ['id', 'highway', 'name'])
        # logging.info('   Straßenreferenz transformieren...')
        ...
        logging.info('   Straßenmöbel einladen...')
        if not layer_raw_man_made_points:
            layer_raw_man_made_points = QgsVectorLayer(VariablesAndSettings.data_dir + 'man_made.geojson|geometrytype=Point', 'man_made (raw)', 'ogr')
        ...
        logging.info('   Straßenmöbel reprojizieren...')
        layer_street_furniture = processing.run('native:reprojectlayer', {'INPUT': layer_street_furniture, 'TARGET_CRS': QgsCoordinateReferenceSystem(VariablesAndSettings.crs_to), 'OUTPUT': 'memory:'})['OUTPUT']
        ...
        logging.info('   Relevante Straßen und Wege selektieren...')
        ...
        logging.info('   Wegenetz in Punkte umwandeln...')
        ...
        # logging.info('   Nächstgelegenen Straßenpunkt ermitteln...')

        # Zunächst Verbindung zu nächstgelegener Verkehrsstraße suchen
        logging.info('   An Verkehrsstraßen ausrichten...')
        ...
        logging.info('   An sonstigen Wegen ausrichten...')
        ...
        logging.info('   Verwerfe Mehrfach-Objekte...')
        ...
        logging.info('   Vereinige ausgerichtete Straßenmöbel...')
        ...
        processing.run('native:saveselectedfeatures', {'INPUT': layer_street_furniture, 'OUTPUT': VariablesAndSettings.proc_dir + 'street_furniture.geojson'})

    # Baumkronendurchmesser und Stammumfang abschätzen
    if VariablesAndSettings.proc_trees:
        logging.info('Baumkronendurchmesser abschätzen...')
        # Bäume in separatem Layer speichern
        layer_trees = QgsVectorLayer(VariablesAndSettings.data_dir + 'natural.geojson|geometrytype=Point', 'Bäume', 'ogr')
            ...
            if feature.attribute('natural') == 'tree' or feature.attribute('natural') == 'shrub':
                if diameter == NULL:
                    ...
                    if circumference:
                        ...
                        except:  # noqa  # TODO: add exception type, aka "except Exception as e:" where Exception is ap specific exception-class
        ...
        QgsVectorFileWriter.writeAsVectorFormatV2(layer_trees, VariablesAndSettings.proc_dir + 'trees.geojson', VariablesAndSettings.transform_context, VariablesAndSettings.save_options)

    # Waldbäume über ein hexagonales Gitter interpolieren
    if VariablesAndSettings.proc_forests:
        logging.info('Waldbäume interpolieren...')
        if not VariablesAndSettings.layer_raw_landuse_polygons:
            VariablesAndSettings.layer_raw_landuse_polygons = QgsVectorLayer(VariablesAndSettings.data_dir + 'landuse.geojson|geometrytype=Polygon', 'landuse polgons (raw)', 'ogr')
        if not VariablesAndSettings.layer_raw_natural_polygons:
            VariablesAndSettings.layer_raw_natural_polygons = QgsVectorLayer(VariablesAndSettings.data_dir + 'natural.geojson|geometrytype=Polygon', 'natural (polygon)', 'ogr')

        layer_forest = processing.run('qgis:extractbyexpression', {'INPUT': VariablesAndSettings.layer_raw_landuse_polygons, 'EXPRESSION': '"landuse" = \'forest\'', 'OUTPUT': 'memory:'})['OUTPUT']
        layer_wood = processing.run('qgis:extractbyexpression', {'INPUT': VariablesAndSettings.layer_raw_natural_polygons, 'EXPRESSION': '"natural" = \'wood\'', 'OUTPUT': 'memory:'})['OUTPUT']
        ...
        layer_forest = processing.run('native:reprojectlayer', {'INPUT': layer_forest, 'TARGET_CRS': QgsCoordinateReferenceSystem(VariablesAndSettings.crs_to), 'OUTPUT': 'memory:'})['OUTPUT']
        ...
        logging.info('   Erzeuge hexagonales Gitter...')
        ...
        coords = coords + ' [' + VariablesAndSettings.crs_to + ']'
        layer_forest_grid = processing.run('native:creategrid', {'CRS': QgsCoordinateReferenceSystem(VariablesAndSettings.crs_to), 'EXTENT': coords, 'TYPE': 4, 'HOVERLAY': 0, 'VOVERLAY': 0, 'HSPACING': 6, 'VSPACING': 6, 'OUTPUT': 'memory:'})['OUTPUT']

        logging.info('   Beschneide Gitter auf Waldflächen...')
        layer_forest_grid = processing.run('native:clip', {'INPUT': layer_forest_grid, 'OVERLAY': layer_forest, 'OUTPUT': 'memory:'})['OUTPUT']

        logging.info('   Schrumpfe Gitterfelder...')
        layer_forest_grid = processing.run('native:buffer', {'INPUT': layer_forest_grid, 'DISTANCE': -0.75, 'SEGMENTS': 1, 'OUTPUT': 'memory:'})['OUTPUT']

        logging.info('   Erzeuge zufällige Baumpunkte...')
        ...
        logging.info('   Ermittle Baumeigenschaften...')
    ...
    if VariablesAndSettings.proc_labels:
        logging.info('Erzeuge Segmente für Straßen- und Gewässernamen...')
        if not VariablesAndSettings.layer_raw_highway_ways:
            VariablesAndSettings.layer_raw_highway_ways = QgsVectorLayer(VariablesAndSettings.data_dir + 'highway.geojson|geometrytype=LineString', 'highway (raw)', 'ogr')
        if not VariablesAndSettings.layer_raw_path_ways:
            VariablesAndSettings.layer_raw_path_ways = QgsVectorLayer(VariablesAndSettings.data_dir + 'path.geojson|geometrytype=LineString', 'path (raw)', 'ogr')
        if not VariablesAndSettings.layer_raw_waterway_ways:
            VariablesAndSettings.layer_raw_waterway_ways = QgsVectorLayer(VariablesAndSettings.data_dir + 'waterway.geojson|geometrytype=LineString', 'waterway (raw)', 'ogr')
        layer_bridges = QgsVectorLayer(VariablesAndSettings.data_dir + 'bridge.geojson|geometrytype=Polygon', 'bridge (raw)', 'ogr')
        ...
        layer_streets = processing.run('qgis:extractbyexpression', {'INPUT': VariablesAndSettings.layer_raw_highway_ways, 'EXPRESSION': '"highway" IS NOT \'platform\' and NOT("highway" like \'%_link\') and "highway" IS NOT NULL', 'OUTPUT': 'memory:'})['OUTPUT']

        # Straßen und Wege nach den Merkmalen Name, Klassifikation und Zweirichtungs-Fahrbahn vereinigen
        logging.info('   Straßen nach gemeinsamen Merkmalen vereinigen...')
        layer_streets = processing.run('native:dissolve', {'FIELD': streetname_attributes, 'INPUT': layer_streets, 'OUTPUT': 'memory:'})['OUTPUT']
        layer_ways = processing.run('native:dissolve', {'FIELD': ['name', 'highway', 'dual_carriageway'], 'INPUT': VariablesAndSettings.layer_raw_path_ways, 'OUTPUT': 'memory:'})['OUTPUT']
        ...
        layer_streetnames = processing.run('native:reprojectlayer', {'INPUT': layer_streetnames, 'TARGET_CRS': QgsCoordinateReferenceSystem(VariablesAndSettings.crs_to), 'OUTPUT': 'memory:'})['OUTPUT']
        ...
        logging.info('   Zweirichtungs-Fahrbahnen vereinfachen...')
        ...
        for dual_street in layer_streetnames_dual.getFeatures():
            ...
            layer_buffer = QgsVectorLayer("Polygon?crs=" + VariablesAndSettings.crs_to, "dual_carriageway_buffer", "memory")
        ...
        QgsVectorFileWriter.writeAsVectorFormat(layer_streetnames, VariablesAndSettings.proc_dir + 'street_names.geojson', 'utf-8', QgsCoordinateReferenceSystem(VariablesAndSettings.crs_from), 'GeoJson')

        # für Gewässernamen:
        logging.info('   Gewässerlinien an Brücken segmentieren...')
        # Tunnel ausschließen und Gewässerlinien nach Namen auflösen
        layer_waterways = processing.run('qgis:extractbyexpression', {'INPUT': VariablesAndSettings.layer_raw_waterway_ways, 'EXPRESSION': '"tunnel" IS NULL', 'OUTPUT': 'memory:'})['OUTPUT']
        layer_waterways = processing.run('native:reprojectlayer', {'INPUT': layer_waterways, 'TARGET_CRS': QgsCoordinateReferenceSystem(VariablesAndSettings.crs_to), 'OUTPUT': 'memory:'})['OUTPUT']
        ...
        layer_waterways = processing.run('native:extractbylocation', {'INPUT': layer_waterways, 'INTERSECT': layer_waterways_points, 'PREDICATE': [0], 'OUTPUT': VariablesAndSettings.proc_dir + 'waterway_names.geojson'})
